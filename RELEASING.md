...
Maintainer guide for publishing a new version of `@piwikpro/react-piwik-pro` to npm.
...
Release **after** the PR is merged, so it points at the real merge commit on
`master`.
...
4. Open a pull request titled **`chore(release): x.y.z`** and merge it into
   `master`. This is what lands the changelog and version bump on `master`.
...
6. Publishing the GitHub Release triggers `main.yml`, which builds and publishes
   to npm. Verify at
   <https://www.npmjs.com/package/@piwikpro/react-piwik-pro>.
